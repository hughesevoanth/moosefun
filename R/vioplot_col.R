...
#' @examples
#' vioplot_col()
vioplot_col <- function (x, ..., range = 1.5, h = NULL, ylim = NULL, names = NULL,  horizontal = FALSE, col = rep("magenta", length(list(x, ...))), border = "black", lty = 1, lwd = 1, rectCol = "black", colMed = "white", pchMed = 19, at, add = FALSE, wex = 1, drawRect = TRUE) {
    datas <- list(x, ...)
    n <- length(datas)
    if (missing(at)) 
        at <- 1:n
